 ...
 * @returns Returns an array with keyboard layout properties.
 * @see https://wiki.mtasa.com/wiki/GetKeyboardLayout
 **/
declare function getKeyboardLayout(): any[];
 ...
 **/
declare function getLocalization(): {code: string, name: string};
 ...
 **/
declare function getNetworkStats(): NetworkStatsFormant;

/**
 * This function returns an array containing network usage information about inbound and outbound packets.
 * @returns Returns an array with two fields: "in" and "out". Each of these contain an array with two fields: "bits" and "count". Each of these contain an array with 256 numeric fields ranging from 0 to 255, containing the appropriate network usage data for such packet id.
...
/**
 * This function gets the server or client (if used client sided it returns time as set on client's computer) real time and returns it in an object.
 ...
 * @param algorithm The algorithm to use.
 * @param options object with options for the hashing algorithm,.
